...
``` r
tornado_data<-read_csv("data/stormevents/noaa_stormevent_tornado_2026_08.csv")
```
...
``` r
tornado_data_2<-read_csv("data/SVI_2022_US_county.csv")
```
...
The variable TOR_F_SCALE is ordinal data because it places the magnitude
of a tornado into ranked categories. The variable CZ_TIMEZONEE is
nominal data because months can be put into categories, but they do not
have a rank order. MONTH_NAME would be more efficiently read as a factor
because they can be sorted in chronological order.
...
``` r
filtered_tor<-filter(tornado_data,TOR_F_SCALE=="EF3"|TOR_F_SCALE=="EF4"|TOR_F_SCALE=="EF5"|
                       TOR_F_SCALE=="F3"|TOR_F_SCALE=="F4"|TOR_F_SCALE=="F5")
filtered_tor_summarized<-filtered_tor %>%
  group_by(STATE_FIPS, 
           STATE, 
           cty_fips, 
           CZ_NAME) %>%
...
``` r
filtered_pop<-filter(tornado_data_2,E_TOTPOP>=25000)
```
...
``` r
filtered_pop_vulnerable<-filter(filtered_pop,F_AGE17==1)
```

Describe your variable, why you chose it, and (if applicable) why you
chose the cutoff you did.

The variable that I chose was F_AGE17, which flags areas that are in the
90th percentile for people 17 years and younger. I chose this variable
because children are considered a vulnerable population and require
attention if they are separated from their families during a tornado.
...
``` r
filtered_pop_vulnerable <- filtered_pop_vulnerable %>%
...
``` r
join_tables<-inner_join(filtered_tor_summarized,filtered_pop_vulnerable,by='cty_fips')
```
...
The inner_join() function joins two tables by their matching column. It
will only keep observations from table 1 that have a matching key in
table 2. This differs from the output of a full_join() operation where
all observations in both tables are kept. In a inner_join(), unmatched
rows are not included while in a full_join(), these rows are included.
...
The three counties with the highest number of severe tornadoes are
Mississippi County, Arkansas, with 11, Hall County, Nebraska, with 10,
and Harris County, Texas, with 10. The most notable city in Harris
County is Houston, which has the Buffalo Bayou. Mississippi County in
Arkansas has Blytheville and Osceola, which is as prominent Mississippi
River port that has the southern judicial seat.
...
The paper “A Bayesian Hierarchical Modeling Framework for Correcting
Reporting Bias in the U.S. Tornado Database” uses the dataset to analyze
how there are lower reported tornado counts in rural areas due to fewer
observations. The goal is to improve climate studies and tornado models
for forecasters, planners, and insurance companies. Additionally, this
can be used to strengthen storm-scale prediction systems. The study uses
Bayesian hierarchical modeling, which showed that population density was
a main factor over distance from nearest city, terrain ruggedness index,
and road density. The study also found that approximately 45% of
tornadoes within the analysis domain were reported but the estimated
reporting rate decreased sharply as you move further away from
population centers.
...
While completing this lab, I used input from my classmates as well as
the help feature in R to understand more about the join functions.
Additionally, I used Google’s AI overview to help me understand the
syntax for performing an inner-join().

**Lab reflection:** How do you feel about the work you did on this lab?
Was it easy, moderate, or hard? What are the biggest things you learned
by completing it?

I feel very good about the work I did on this lab. I made sure to take
the time to organize all my code in order for it to be readable. The
code for the lab was moderately easy, but I ran into issues
understanding the variables in the data set and how I can best use it.
For example, I found it difficult categorizing the variables into the
